...
1. Install the published package from npm (recommended):
   ...
   The current `latest` version is `0.1.0`; see the [npm package](https://www.npmjs.com/package/@mymeter/dsh-cost-meter).
...
- `@mymeter/dsh-cost-meter@0.1.0` is published to the npm registry. New profiles can install it directly, and existing installations can use the loopback Web UI update check. Self-update still requires a `file:` baseUrl that resolves to the local dsh profile directory.
