...
1. 直接从 npm 安装正式发布包（推荐）：
   ...
   当前 `latest` 版本为 `0.2.1`，详见 [npm package](https://www.npmjs.com/package/@mymeter/dsh-cost-meter)。
...
- `@mymeter/dsh-cost-meter@0.2.1` 已发布到 npm registry；新 profile 可直接使用 npm 安装，已安装用户可在 loopback Web UI 中检查更新。自更新仍要求当前 dsh profile 使用可解析本地目录的 `file:` baseUrl。
